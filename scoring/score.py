...
def is_process_alive(pid):
    try:
        ...
        logger.debug(f"Process {ingestion_pid} is alive")
        return False
    else:
        logger.debug(f"Process {ingestion_pid} is not alive")
        return True
...
if __name__ == "__main__":
    ...
    logger.debug("Successfully get the metric")
    ...
    try:
        ...
        scores_so_far = []
        num_preds = 0
        logger.debug("Enter cycle to check prediction")
        while (ingestion_is_alive(prediction_dir) and
                is_process_alive(ingestion_pid)):
            time.sleep(1)
            # Get list of prediction files
            logger.debug("In cycle")
            prediction_files = get_prediction_files(prediction_dir)
            logger.debug("Get all prediction files")
            num_preds_new = len(prediction_files)
            logger.debug("Get num of new predictions")
