    ...
    def init_model(self,
        ...
        for layer in range(1, layers):
            ...
            x = Convolution2D(
                ...
                kernel_size=kernel_size,
                padding='same',
                name=f'conv{layer + 1}_1')(x)
            x = ELU()(x)
            x = BatchNormalization(
                axis=channel_axis,
                name=f'bn{layer + 1}_1')(x)

            x = Convolution2D(filters=filters_size[layer], kernel_size=kernel_size, padding='same',
                              name=f'conv{layer + 1}_2')(x)
            x = ELU()(x)
            x = BatchNormalization(
                axis=channel_axis,
                name=f'bn{layer + 1}_2')(x)

            if filters_size[layer] != 128:
                x = Convolution2D(filters=filters_size[layer], kernel_size=kernel_size, padding='same',
                                  name=f'conv{layer + 1}_3')(x)
                x = ELU()(x)
                x = BatchNormalization(
                    axis=channel_axis, name=f'bn{layer + 1}_3')(x)
