...
class DataManager:
    ...
    def _init_each_class_index(self):
        ...
        log(f'Raw train data: train_num(without val) {len(self._train_y)}; '
            f'class_num {self._num_classes} ; max_class_num {self._max_class_num}; min_class_num {self._min_class_num}; ')
    ...
    def _get_preprocess_train(self, sample_index, pre_func):
        ...
        log(f'Total {len(sample_index)}, update {len(need_pre)}')
        # update
        cnt = 0
        for i in need_pre:
    ...
    def get_train_data(self, train_loop_num, model_num,
        ...
        #     log(f'Data augmention, from {cnt} to {len(train_x)} samples')
    ...
    def get_augmention_data(self, x, y, ratio=0.2):
        ...
        for x_a, y_a in augmention_data:
            # log(f'x {x.shape} y {y.shape} x_a {np.array(x_a).shape} y_a {y_a.shape}')
